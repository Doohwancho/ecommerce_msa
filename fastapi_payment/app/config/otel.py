...
# pip install opentelemetry-instrumentation-fastapi opentelemetry-instrumentation-kafka opentelemetry-instrumentation-sqlalchemy
from opentelemetry.instrumentation.fastapi import FastAPIInstrumentor
# gRPC 클라이언트 계측기는 니 코드에서 주석 처리되어 있으니 그대로 둠
# from opentelemetry.instrumentation.grpc import GrpcInstrumentorClient
from opentelemetry.instrumentation.kafka import KafkaInstrumentor
from opentelemetry.instrumentation.sqlalchemy import SQLAlchemyInstrumentor

# !!! OTel 로깅 관련 임포트 추가 !!!
# pip install opentelemetry-instrumentation-logging opentelemetry-exporter-otlp 필요하다!
from opentelemetry._logs import set_logger_provider
from opentelemetry.sdk._logs import LoggerProvider
from opentelemetry.sdk._logs.export import BatchLogRecordProcessor
from opentelemetry.exporter.otlp.proto.grpc._log_exporter import OTLPLogExporter

# !!! 리소스 임포트 (트레이싱/로깅 같이 쓴다) !!!
from opentelemetry.sdk.resources import Resource

# !!! OTel 로깅 계측 라이브러리 임포트 !!!
...
OTEL_SERVICE_NAME = os.getenv("OTEL_SERVICE_NAME", "payment-service") # Payment 서비스 이름
...
def setup_telemetry():
    ...
    try:
        ...
        resource = Resource.create({
            ...
            # 다른 리소스 속성들 (예: deployment.environment 등) 추가 가능
            # 예: "deployment.environment": os.getenv("ENVIRONMENT", "development"),
            # 예: "service.instance.id": socket.gethostname() # Pod 이름 등 고유 ID (필요시 socket 임포트)
        })
        ...
        # 이놈이 니 logger.info 같은 호출 가로채서 Trace ID/Span ID 붙여준다
        # LoggingInstrumentation().enable()
        ...
        # gRPC 클라이언트 (Payment 서비스는 gRPC 클라이언트 안 쓰는 것 같으니 주석 그대로 둠)
        # from opentelemetry.instrumentation.grpc import GrpcInstrumentorClient
        # GrpcInstrumentorClient().instrument()

        # DB 계측기 (SQLAlchemy)
        # 이놈이 DB 호출 스팬을 만들고, 로깅 계측기는 DB 호출 중 찍힌 로그에 Trace ID 붙인다
        SQLAlchemyInstrumentor().instrument()

        # 메시징 계측기 (Kafka)
        # 이놈이 Kafka 상호작용 스팬을 만든다
        KafkaInstrumentor().instrument()

        # 필요한 다른 계측기들도 여기에 추가 (예: 외부 PG사 호출하는 HTTP 클라이언트 계측기 등)
        # from opentelemetry.instrumentation.httpx import HTTPXClientInstrumentor # 만약 httpx 쓴다면
        # HTTPXClientInstrumentor().instrument()


        logger.info("OpenTelemetry tracing and logging instrumentation setup completed successfully for payment-service")

    except Exception as e:
        logger.error(f"Failed to setup OpenTelemetry instrumentation for payment-service: {str(e)}")
