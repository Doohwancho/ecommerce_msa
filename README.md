...
5. ?. Q. 컨슈머 그룹 리밸런싱 전략: Kafka 컨슈머 리밸런싱 시 발생할 수 있는 지연을 최소화하는 전략이 필요합니다.
6. ?. Q. 오프셋 관리: 자동 커밋 vs 수동 커밋 전략을 명확히 정의하면 좋겠습니다.
7. Q. 분산 시스템에서 트랜젝션 방법은?
    1. 분산 시스템에서 transaction & rollback을 어떻게 처리하지?
    2. 결과 (mermaid flowchart)
8. Q. 상품재고의 overselling 문제를 어떻게 해결하지?
    1. overselling 문제
    2. solution: 예약 재고 수량 트래킹
    3. 재고량 & 예약재고량을 nosql에서 트래킹하는게 맞을까?
    4. 도입
9. Q. ecommerce에서 상품의 read optimization? (yet)
    ...
    4. elastic search에 성능 듀닝한 다음, 튜닝 전과 쿼리 latency 비교
        ...
        4. 분석기 최적화
10. Q. 상품 검색기능 방법? (yet)
    1. elastic search의 특징인 역인덱스가 왜 다른 DB대비 상품 "검색"(full text search)에 유리한지 설명
    2. 상품 데이터 넣을 때 어떻게 변환해서 넣어야 효율적이게 read할 수 있을지 설계하고 넣기 (ex. 키워드들이 여러 필드에 흩어져있는걸 하나의 필드로 합치기)
        - 관련 필드를 단일 검색 필드로 통합
        - 관련성을 위한 필드 부스팅 구현
    3. 다양한 타입의 ES 토크나이저 나열하고 써보기
        1. Standard, N-gram, Edge n-gram 토크나이저 구현
        2. 커스텀 토크나이저 개발
        3. 각 토크나이저별 성능 및 정확도 비교
    4. text analysis (nori 한국어 분석기)? 사용
        1. Nori 분석기 설정 및 구성
        2. 한국어 상품 데이터로 테스트
        3. 한국어 특화 텍스트 패턴 최적화
    5. nori안에 동의어사전,
...
11. Q. 상품 추천 방법? (yet)
    ...
    5. 추천 유사도는 어떤 기준으로 하지?
        ...
        - 추천 다양성 테스트
        - 유사성 알고리즘 최적화
12. Q. k8s container의 안정성을 높히는 방법은? (yet)
    ...
    5. tracing은 jaeger 설치 후, 일부러 에러 터지게 한 다음, distributed tracing 해보기
        - OpenTelemetry SDK로 서비스 계측
        - 샘플 오류 시나리오 생성
        - 트레이스 정보 분석
    6. HA:
        - 마이크로서비스를 위한 여러 레플리카 구성
...
# ai to ask
ecommerce MSA를 만들거야.
내가 쓴 혹은 쓰려고 할 tech stack을 적어놨어.
그리고 TODO에는 내가 여태껏 한 것들(1~6)와, 앞으로 할 것들 (yet)이 적혀있어.

이 문서를 보고, 너가 판단하기에 여태껏 한 것들 중에 수정할 부분이나 추가하면 좋을게 있으면 알려줘.
앞으로 할 것들 중에 어떤 goal을 이루기 위해 어떤 tech stack으로 어떤 방법론을 쓰며 level design으로 step by step으로 어떻게 이룰지, 각 step마다 예상 소요시간은 어떻게 되는지 적어줘.

참고로 내가 생각한 대략적인 방안은 TODO에 각 step 밑에 리스트로 적어둘게.
...
1. Q. 데이터를 어떻게 저장하지?
  ...
  6. update 즉시 적용 vs eventual consistency
  7. 상품 데이터는 어느 DB에 어느 데이터 구조로 저장하지?
  8. 상품 카테고리 정보는 어디에 저장하는게 좋을까?
  9. 상품 카테고리 정보는 어떻게 저장하지?
2. Q. MSA 환경에서 컨테이너들이 어떻게 통신하지?
  1. MSA 컨테이너 간 통신 프로토콜 비교 (JSON vs HTTPx vs gRPC)
3. Q. 동기? 비동기?
  1. create_order()에 전체 흐름 파악
...
7. Q. ecommerce에서 상품의 read optimization? (yet)
  ...
  3. mongodb에 쿼리
8. Q. 상품 검색기능 방법? (yet)
  1. elastic search의 특징인 역인덱스가 왜 다른 DB대비 상품검색에 유리한지 설명
  2. 다양한 타입의 ES 토크나이저 나열하고 써보기
  3. text analysis (nori)? 사용
  4. nori안에 동의어사전, 유사어 검색 사전 이용해보기
  5. 상품 데이터 넣을 때 어떻게 변환해서 넣어야 효율적이게 read할 수 있을지 설계하고 넣기 (ex. 키워드들이 여러 필드에 흩어져있는걸 하나의 필드로 합치기)
9. Q. 상품 추천 방법? (yet)
  - 아주 간단한 기초 모델인
10. Q. k8s container의 안정성을 높히는 방법은? (yet)
  1. open telemetry를 표준 삼아서, logging은 beats + elastic search + kibana에, 모니터링은 prometheus + grafana에, tracing은 jaeger로
  2. jaeger 설치 후, 일부러 에러 터지게 한 다음, distributed tracing 해보기
  3. HA로 fastapi_modules들(user, product, order, payment) 2개 이상 띄우고, 하나가 에러나면 다른하나에 로드밸런서로 트래픽 쏠리게끔 유도하기
11. Q. 무중단으로 자동화 배포하는 방법은? (yet)
  1. 로컬에 jenkins 서버 띄워보기
  2. terraform으로 aws-ec2하나 만들어서 jenkins 서버 만들고 jenkins 서버 띄워보기
  3. user module에 간단한 unit test하나 만들고, fastapi, mongodb 이거 두개만 eks로 띄워보기
  4. mongodb가 무중단 하면서 배포하는걸 테스트해보기
...
## c. 100만건의 상품을 mongodb에서 stress test
...
## d. 100만건의 상품을 elastic search에서 stress test
```bash
